...
void MugParticleObject::create(ParticleObjectDetais details, int& numOfParts)
{
	...
	const float thickness = max(details.thickness, Configuration.GLASS_PARTICLE_BUILD_GAP);
	...
	float outputThickness = thickness;
	...
	ParticleGeometry::filledCircle(numOfParts
					...
					, innerRadius, innerRadius+1, ParticleGeometry::EQUALS	// outer
	...
	ParticleGeometry::cylinder(numOfParts
					...
					, bottom, top - layerGap/2		// heights
					, innerRadius	// radius
					, inCircleGap, layerGap, ParticleGeometry::INSIDE
					, { ParticleGeometry::INSIDE, ParticleGeometry::INSIDE, ParticleGeometry::INSIDE }
					, { ParticleGeometry::UP_INSIDE, ParticleGeometry::UP_INSIDE, ParticleGeometry::INSIDE });

	ParticleGeometry::filledCircle(numOfParts
		, topVec
		, innerRadius + layerGap / 2, innerRadius, ParticleGeometry::EQUALS	// inner
		, innerRadius + thickness - layerGap / 2, innerRadius + thickness, ParticleGeometry::EQUALS	// outer
		, inCircleGap, layerGap, ParticleGeometry::UP
		, { ParticleGeometry::UP, ParticleGeometry::UP, ParticleGeometry::UP });

	ParticleGeometry::cylinder(numOfParts
		, glm::vec2(localCenter.x, localCenter.z)
		, bottom - thickness, top - layerGap / 2		// heights
		, innerRadius + thickness	// radius
		, inCircleGap, layerGap, ParticleGeometry::OUTSIDE
		, { ParticleGeometry::OUTSIDE, ParticleGeometry::OUTSIDE, ParticleGeometry::OUTSIDE }
		, { ParticleGeometry::OUTSIDE, ParticleGeometry::OUTSIDE, ParticleGeometry::OUTSIDE });

	ParticleGeometry::filledCircle(numOfParts
		, bottomVec - glm::vec3(0, thickness, 0)
		, 0, 0, ParticleGeometry::EQUALS		// inner
		, innerRadius + thickness, innerRadius + thickness + 1, ParticleGeometry::EQUALS	// outer
...
//
//void create_OLD(ParticleObjectDetais details, int& numOfParts)
//{
//	const float inCircleGap = Configuration.GLASS_PARTICLE_BUILD_GAP; // gap on oX, in a circle
//	const float layerGap = (float)inCircleGap * (float)sqrt(3) * 0.5f;	// gap on oY
//	const float innerRadius = details.innerRadius;
