...
public:
	PyramidPointerMaterialObject(ShaderProgram ShaderProgram, float PyramidColor[3], unsigned int BaseVertices, unsigned int PyramidHeight, unsigned int PyramidRadius)
		...
		this->Vertices = new float[(BaseVertices + 2) * 3]; // create room in memory for n base vertices + top vertex + bottom center vertex, all multiplied by 3 (coordinates)
	}
	void init() override;
...
private:
	...
	float* CalculateBaseVertexCoordinateForIndex(unsigned int Index, unsigned int BaseVertices, unsigned int PyramidRadius, unsigned int PyramidHeight) const;
